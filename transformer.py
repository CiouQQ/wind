...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        attention = torch.softmax(energy / (self.embed_dim ** (1 / 2))*1.0, dim=3)
        # attention = self.dropout(torch.softmax(energy / (self.embed_dim ** 0.5), dim=3))
...
class TransformerBlock(Module):
    def __init__(self, embed_dim, num_heads, config):
        ...
        self.fc = nn.Sequential(nn.Linear(embed_dim, embed_dim), nn.LeakyReLU(negative_slope=0.01))
