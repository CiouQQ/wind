...
class ChinesePostman:
    ...
    def step(self, action):
        ...
        if self.map[self.current_node][action] > 0: #
            if self.traveled[self.current_node][action] == 0.0 : #
                reward = reward + 10
                self.traveled_num += 1
                self.update_traveled_edges(action)
    ...
    def find_untraveled_edges(self):
        untraveled_edges = []
        for i in range(self.num_nodes):
            for j in range(i + 1, self.num_nodes):  # 嚙質器��硺�𤏸𨯙嚙賡憫��躰都�曉�躰都鞎滚�躰�肽𨯙嚙踢䁥滩𨯙��𢠃陷�𨯙��躰陪薴ㄟ��躰�肽𨯙��辷蕭
    ...
    def manage_path_to_untraveled_edge(self):
        if self.target_path is None or not self.target_path:
            ...
            if isinstance(path, list):  # 嚙踝蕭臐�梹蕭��𡏭𨯙嚙質�肽𨯙��嚙賡迚�𨯙嚙賡�貉𨯙��躰�頣蕭璈坿𨯙��躰閉嚙踝蕭�𨯙��躰都嚙質�莞����辷蕭
