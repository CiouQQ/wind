...
import time
NUM_NODES = 9
Side = int(NUM_NODES**0.5) #model.py
...
class ChinesePostman:
    def __init__(self, num_nodes=NUM_NODES):
        ...
        self.current_node = random.randint(0, 0)
        self.deopt = self.current_node
        self.num_edges_to_remove = random.randint(0, 3)
    ...
    def generate_map(self):
        for i in range(self.num_nodes):
            if i % Side != Side - 1:
                self.map[i][i + 1] = self.map[i + 1][i] = random.randint(7, 13)
            if i < Side * (Side - 1):
                self.map[i][i + Side] = self.map[i + Side][i] = random.randint(7, 13)
    ...
    def step(self, action):
        ...
        if self.map[self.current_node][action] > 0: #有這個路線
            if self.traveled[self.current_node][action] == 0.0 : #這個路線沒被走過
                ...
                # reward = reward-500
                reward = reward - self.map[self.current_node][action] * self.traveled[self.current_node][action]*10
                self.traveled[self.current_node][action] = self.traveled[self.current_node][action] + 1
                self.traveled[action][self.current_node] = self.traveled[self.current_node][action] 
            else:
                reward = reward-1000
