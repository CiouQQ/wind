...
class ChinesePostman:
    ...
    def observation_space(self):
        observation_space = spaces.Box(low=-np.finfo(np.float32).max,
                               high=np.finfo(np.float32).max,
                               shape=(163,), dtype=np.float32)
    ...
    def reset(self):
        ...
        combined_map_flattened = combined_map.flatten()
        combined_map_flattened = np.insert(combined_map_flattened, 0, self.current_node)
        return combined_map_flattened
    ...
    def step(self, action):
        ...
        combined_map = np.concatenate((self.map, self.traveled/100), axis=0)
        combined_map_flattened = combined_map.flatten()
        combined_map_flattened = np.insert(combined_map_flattened, 0, self.current_node)
        # print(combined_map_flattened)
        return combined_map_flattened, reward, done, info
    ...
    def render(self):
        input()
        print(f"Now is {self.current_node}")
        print("Need to ",self.need_to_travel,"Now :",self.traveled_num)
