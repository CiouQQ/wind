...
class ChinesePostman:
    def __init__(self, num_nodes=NUM_NODES):
        ...
        self.mask = np.ones_like(self.map_model, dtype=bool)
        np.fill_diagonal(self.mask, False)
        self.zero_mask = (self.map_model == 0) & self.mask
        self.map_model[self.zero_mask] = -1
        self.traveled[self.zero_mask] = -1
        self.ok = 0
    ...
    def step(self, action):
        ...
        if self.map_model[self.current_node][action] > 0: #
            if self.traveled[self.current_node][action] == 0.0 : #
                ...
                reward = reward - (self.map_model[self.current_node][action] * self.traveled[self.current_node][action] / 2)
                self.traveled[self.current_node][action] = self.traveled[self.current_node][action] + 1
                self.traveled[action][self.current_node] = self.traveled[self.current_node][action] 
            else:
                reward = reward-10
    ...
    def find_untraveled_edges(self):
        untraveled_edges = []
        for i in range(self.num_nodes):
            for j in range(i + 1, self.num_nodes):  # 嚙踝蕭頨圈鐯嚙質�亙�躰�肽𨯙��𣇉飭�𨯙�𩸭�艔��嚙賢�䈑蕭頨圈�剁蕭�祈𨯙��躰釭皛䕘蕭頦肽𨯙嚙踝蕭���𨯙�礆���蕭鞈∴蕭�𩣪嚙踢�航𨯙�礆���𨯙嚙質腺颲𥕦�䈑蕭頨啗�綽蕭鈭方�滩�嚙質噸�𨯙��躰腺嚙踝蕭嚙質���躰腺颲𥡝噸�𨯙��躰釭皛䕘蕭��嚙踢弩哨蕭嚙質𨯙��躰釭皛��躰釭嚙質腺颲𥡝噸�𨯙��躰釭�烵嚙踢辛�
    ...
    def manage_path_to_untraveled_edge(self):
        if self.target_path is None or not self.target_path:
            ...
            if isinstance(path, list):  # 嚙踝蕭頨堆蕭�踢辛辷蕭嚙賜�𡏭𨯙���砥藂�坔�躰�肽𨯙��𣂼噡��嚙賢�䈑蕭頨圈鐯��躰釭嚙質腺颲𥡝噸�𨯙��躰郭嚙質滌�券���航𨯙��嚙賢�䈑蕭頨圈�典�躰釭嚙賢�栞�𥡝噸�𨯙��躰釭皛��躰部餈𡄯蕭藂�辷蕭嚙踝蕭頦舘�嚙質噸�𨯙��躰釭皛䕘蕭嚙賢爀嚙質滌嚙質�踢辛坔�躰腺颲𥡝噸�𨯙��躰釭皛䕘蕭鞈ｇ蕭頨圈鐯��躰釭嚙賜𣵛�𨯙��躰�肽𨯙��躰釭�烵嚙踢辛�
